...
use Carbon\Carbon;
use App\Models\OwnerForecastingMenu;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class OwnerForecastingController extends Controller
{
    public function ownerForecasting()
    {
        ...
        $menuNames = OwnerForecastingMenu::orderBy('menu_name', 'asc')->pluck('menu_name', 'menu_id')->toArray();

        return view('owner_forecasting', compact('weeklyData', 'monthlyData', 'quarterlyData', 'yearlyData', 'menuNames'));
    }

    private function getWeeklyMenuSales(): array
    {
        $oldestOrderTimestamp = DB::table('reseller_order')->min('order_timestamp');
        ...
        while (true) {
            ...
            $hasOrders = DB::table('reseller_order')
            ...
            $orders = DB::table('reseller_order')
                ->whereBetween('order_timestamp', [$weeklyStartDate, $endDate])
                ->join('reseller_order_information', 'reseller_order.order_id', '=', 'reseller_order_information.order_id')
                ->select('reseller_order_information.menu_id', DB::raw('SUM(reseller_order_information.quantity) as quantity'))
                ->groupBy('reseller_order_information.menu_id')
                ->get();
    ...
    private function getMonthlyMenuSales(): array
    {
        ...
        $allMenuIds = OwnerForecastingMenu::pluck('menu_id')->toArray(); // Get all menu IDs
