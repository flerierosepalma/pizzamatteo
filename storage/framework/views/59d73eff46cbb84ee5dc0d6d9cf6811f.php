...
<head>
    ...
    <title>Pizza Matteo | Stock Request</title>
    ...
    <style>
       ...
       #sendStock  .quantity-counter button {
            padding: 2px;
            border: 1px solid #ccc;
            cursor: pointer;
            border-radius: 50%;
        ...
        #sendStock  .quantity-counter input {
            width: 40px;
            text-align: center;
            border: none;
            margin-left: 10px;
        ...
        #sendStock .send-close-btn:hover {
            background-color: #8A1C14;
            color: #fff;
            outline: none;
        }

        #sendStock .order-item {
            text-align: center;
            padding: 8px 0;
        }

        #sendStock .order-item h5 {
            font-size: 0.9rem;
            font-weight: bold;
            margin-bottom: 3px;
        }

        #sendStock .order-item .item-details {
            display: flex;
            justify-content: space-between;
            font-size: 0.8rem; 
        }

        #sendStock .order-item .item-details p {
            margin: 3px 0;
        }

        #sendStock .modal-body {
            text-align: center;
        }

        #sendStock .stock_details {
            width: 100%;
            border-collapse: collapse;
            margin-top: 10px;
            box-shadow: none; 
        ...
        #sendStock .stock_details th,
        #sendStock .stock_details td {
        ...
        #sendStock .stock_details input[type="number"] {
            width: 40px;
            padding: 4px;
            box-sizing: border-box;
            font-size: 0.8rem;
        ...
        #sendStock .total-order h5 {
            font-size: 0.8rem;
            font-weight: bold;
        }

        #sendStock .total-order span {
            font-size: 0.8rem;
            font-weight: normal;
        }
        #sendStock .send-button-container {
            display: flex;
            justify-content: center;
            padding: 10px 0;
        }

        #sendStock .send-button {
            background-color: #8A1C14;
            color: #fff;
            border: none;
            padding: 10px 20px;
...
#completeSummary .complete-close-btn {
    position: absolute;
    top: 10px;
    right: 10px;
    width: 30px;
...
#completeSummary .complete-close-btn:hover {
    background-color: #8A1C14;
    color: #fff;
    outline: none;
}
...
#completeSummary .order-item {
    border-bottom: 1px solid #ddd;
    padding: 8px 0;
}

#completeSummary .order-item:last-child {
    border-bottom: none; 
}

#completeSummary .order-item h5 {
    font-size: 0.8rem;
    font-weight: bold;
    margin-bottom: 3px;
}

#completeSummary .order-item .item-details {
    display: flex;
    justify-content: space-between;
    font-size: 0.7rem; 
}

#completeSummary .order-item .item-details p {
    margin: 3px 0;
}

#completeSummary .total-order {
    padding-top: 15px;
    margin-bottom: 15px;
    text-align: right;
    border-top: 1px solid #ddd;
...
#completeSummary .total-order h5 {
    font-size: 0.8rem;
    font-weight: bold;
}

#completeSummary .total-order span {
    font-size: 0.8rem;
    font-weight: normal;
}
#completeSummary .stock-details-table {
    width: 100%;
    border-collapse: collapse;
    margin-top: 10px;
}

#completeSummary .stock-details-table th, .stock-details-table td {
    border: 1px solid #ddd;
    padding: 8px;
}

#completeSummary .stock-details-table th {
    background-color: #f2f2f2;
    text-align: center;
}
...
<body>
    ...
    <section class="home">
        <div class="top">
            ...
            <span class="page-title">Stock Request</span>
                    ...
                    <tbody id="table-1-body">
                        <?php $__currentLoopData = $pendingOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr>
                                ...
                                <td><?php echo e(\Carbon\Carbon::parse($order->order_timestamp)->format('F j, Y, g:i A')); ?></td>
                                <td>
                                    <button class="btn-summary" onclick="showSummaryModal('<?php echo e($order->order_id); ?>')">See Order</button>
                                </td>
                                <td>
                                    <?php if($order->payment_proof): ?>
                                        <button class="btn-proof" onclick="showPaymentProofModal('<?php echo e(asset('storage/' . $order->payment_proof)); ?>')">View Proof</button>
                                    <?php else: ?>
                                ...
                                <td>
                                    <button class="btn-cancel" data-order-id="<?php echo e($order->order_id); ?>">Cancel</button>
                        <button class="btn-accept" data-order-id="<?php echo e($order->order_id); ?>">Accept</button>
                                </td>
                            </tr>
            ...
            <tbody id="table-2-body">
                <?php $__currentLoopData = $preparingOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr>
                        ...
                        <td>₱<?php echo e($order->order_total_amount); ?></td>
                        <td>
                            <button class="btn-send" onclick="showSendStockModal('<?php echo e($order->order_id); ?>')">Send Stock</button> <!-- Button to trigger modal for testing -->
                        </td>
                    </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </tbody>
                    ...
                    <tbody id="table-3-body">
                        <?php $__currentLoopData = $completedOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr>
                                ...
                                <td><?php echo e(\Carbon\Carbon::parse($order->order_timestamp)->format('F j, Y, g:i A')); ?></td>
                                <td>
                                    <button class="btn-summary" onclick="showCompleteSummaryModal('<?php echo e($order->order_id); ?>')">See Order Summary</button>
                                </td>
                                <td>
                                    <?php if($order->payment_proof): ?>
                                        <button class="btn-proof" onclick="showPaymentProofModal('<?php echo e(asset('storage/' . $order->payment_proof)); ?>')">View Proof</button>
                                ...
                                <td>₱<?php echo e($order->order_total_amount); ?></td>
                                    <td><?php echo e(\Carbon\Carbon::parse($order->order_completed)->format('F j, Y, g:i A')); ?></td>
                            </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </tbody>
                </table>
...
<!-- ============= ORDER SUMMARY MODAL =============== -->
        <div class="modal" id="orderSummary" style="display: none;">
...
<!-- ============= SEND STOCK MODAL =============== -->
        <div id="sendStock" class="modal" style="display: none;">
            <div class="modal-content">
                <span class="send-close-btn">&times;</span>
                <h2>Send Stock <span id="sendStockOrderId"></span></h2>
                <div id="sendStockContent"></div>
...
<div class="modal" id="completeSummary" style="display: none;">
    <div class="modal-content">
        <span class="complete-close-btn" onclick="closeCompleteSummaryModal()">&times;</span>
        <h2>Complete Summary</h2>
        <div class="modal-body">
            <div id="completeSummaryContent">
                <!-- Order details dynamically populated here -->
            </div>
            <div class="total-order">
                <h5>Total Order Amount: <span id="totalOrderAmount"></span></h5>
            </div>
...
<script>
function showCompleteSummaryModal(orderId) {
    $.ajax({
        url: '<?php echo e(route("get.complete.summary")); ?>',
        type: 'GET',
        ...
        success: function(response) {
            ...
            var menuGroupedDetails = {};

            // Group order details by menu_name
            $.each(orderDetails, function(index, order) {
                if (!menuGroupedDetails[order.menu_name]) {
                    menuGroupedDetails[order.menu_name] = {
                ...
                if (order.stock_item_id) {
                    menuGroupedDetails[order.menu_name].stocks.push({
                        stock_item_id: order.stock_item_id,
                        total_stock: order.total_stock,
                        stock_date: order.stock_date,
            ...
            var html = '';
            $.each(menuGroupedDetails, function(menu_name, details) {
                html += '<div class="order-item">';
                html += '<h5>' + menu_name + '</h5>';
                html += '<div class="item-details">';
                html += '<p>Quantity: ' + details.quantity + '</p>';
                html += '<p>Amount: ₱' + details.amount + '</p>';
                html += '</div>';
                // Display reseller stock details in a table
                if (details.stocks.length > 0) {
                    html += '<table class="stock-details-table">';
                    html += '<tr><th>Stock Item ID</th><th>Total Stock</th><th>Stock Date</th><th>Expiry Date</th></tr>';
                    $.each(details.stocks, function(index, stock) {
                        html += '<tr>';
                        html += '<td>' + stock.stock_item_id + '</td>';
                        html += '<td>' + stock.total_stock + '</td>';
                        html += '<td>' + stock.stock_date + '</td>';
                        html += '<td>' + stock.expiry_date + '</td>';
                        html += '</tr>';
                    });
                    html += '</table>';
                } else {
                    html += '<p>No stock details available</p>';
                }

                html += '</div>';
            });

            $('#completeSummaryContent').html(html);
            $('#totalOrderAmount').text('₱' + totalOrderAmount);
            $('#completeSummary').css('display', 'flex');
        },
...
$(document).ready(function() {
    ...
    $('.complete-close-btn').click(function() {
        closeCompleteSummaryModal();
    });

    // Close modal when clicking outside the modal
    $(document).mouseup(function(e) {
        var modal = $('#completeSummary .modal-content');
    ...
    $('#completeSummary').css('display', 'none'); 
});

function closeCompleteSummaryModal() {
    $('#completeSummary').css('display', 'none'); 
}
 ...
 //============= CANCEL/ACCEPT BUTTON ============ //
    $(document).ready(function() {
        $('.btn-cancel').on('click', function() {
            ...
            updateOrderStatus(orderId, 'Owner');
        });
        ...
        function updateOrderStatus(orderId, status) {
            if (confirm("Are you sure you want to update the order status?")) {
                $.ajax({
                    url: "<?php echo e(route('update.reseller.order.status')); ?>",
                    type: 'POST',
                    data: {
...
function showSummaryModal(orderId) {
    $.ajax({
        url: '<?php echo e(route("get.order.summary")); ?>',
        type: 'GET',
        data: { orderId: orderId },
        dataType: 'json',
        success: function(response) {
 ...
 //============= SEND STOCK MODAL ============ //
function changeQuantity(element, delta, maxQuantity, menuId, orderItemId) {
    var quantityInput = $(element).siblings('.quantity-input');
    var currentQuantity = parseInt(quantityInput.val());
    var newQuantity = currentQuantity + delta;

    var totalQuantityInput = $(`.total-quantity[data-menu-id="${menuId}"]`);
    var totalQuantity = parseInt(totalQuantityInput.attr('data-total'));

    var availableStock = parseInt($(element).closest('tr').find('.available-stock').text());

    if (newQuantity >= 0 && newQuantity <= availableStock && totalQuantity + delta <= maxQuantity) {
        quantityInput.val(newQuantity);
        totalQuantityInput.attr('data-total', totalQuantity + delta);
        totalQuantityInput.text(`Total: ${totalQuantity + delta}/${maxQuantity}`);
    }
}


function showDataInModal(orderId, targetContentId) {
    $.ajax({
        url: '<?php echo e(route("get.order.summary.and.inventory")); ?>',
        type: 'GET',
        data: { orderId: orderId },
        dataType: 'json',
        success: function(response) {
            var orderDetails = response.orderDetails;
            var totalOrderAmount = response.totalOrderAmount;
            var inventoryData = response.inventoryData;

            var html = '';
            $.each(orderDetails, function(index, order) {
                html += '<div class="order-item" data-order-id="' + order.order_id + '">';
                html += '<h5>' + order.menu_name + '</h5>';
                html += '<div class="item-details">';
                html += '<div class="total-quantity" data-menu-id="' + order.menu_id + '" data-total="0">Quantity: 0/' + order.quantity + '</div>';
                html += '<p>Amount: ₱' + order.amount + '</p>';
                html += '</div>'; 

                var inventoryItems = inventoryData[order.menu_id] || [];
                if (inventoryItems.length === 0) {
                    html += '<p style="color: red;">No Stock</p>';
                } else {
                    html += '<table class="stock_details">';
                    html += '<thead>';
                    html += '<tr>';
                    html += '<th>Stock ID</th>';
                    html += '<th>Available Stock</th>';
                    html += '<th>Send</th>';
                    html += '</tr>';
                    html += '</thead>';
                    html += '<tbody>';
                    ...
                    $.each(inventoryItems, function(index, item) {
                        html += '<tr>'; 
                        html += '<td>' + item.stock_item_id + '</td>';
                        html += '<td class="available-stock">' + item.inventory_stock + '</td>';
                        html += '<td>';
                        html += '<div class="quantity-counter">';
                        html += '<button type="button" class="quantity-decrement" onclick="changeQuantity(this, -1, ' + order.quantity + ', ' + order.menu_id + ')">-</button>';
                        html += '<input type="number" name="quantities[' + item.menu_id + ']" class="quantity-input" value="0" readonly ';
                        html += 'data-order-item-id="' + order.order_item_id + '" '; 
                        html += 'data-stock-item-id="' + item.stock_item_id + '" ';
                        html += 'data-menu-id="' + order.menu_id + '" ';
                        html += 'data-stock-date="' + item.stock_date + '" ';
                        html += 'data-expiry-date="' + item.expiry_date + '" ';
                        html += 'data-reseller-id="' + order.reseller_id + '" />'; 
                        html += '<button type="button" class="quantity-increment" onclick="changeQuantity(this, 1, ' + order.quantity + ', ' + order.menu_id + ')">+</button>';
                        html += '</div>';
                        html += '</td>';
                        html += '</tr>'; 
                    });
            ...
            $(targetContentId).html(html);
            $('#totalOrderAmount').text(totalOrderAmount); 

            $(targetContentId).closest('.modal').css('display', 'flex');

            attachModalEventHandlers(targetContentId);
        },
        error: function(xhr, status, error) {
    ...
    function showSendStockModal(orderId) {
        showDataInModal(orderId, '#sendStockContent');
    }

    function closeSendStockModal() {
        $('#sendStock').css('display', 'none'); 
    }

    function attachModalEventHandlers(targetContentId) {
        $(targetContentId).closest('.modal').find('.send-close-btn').click(function() {
            closeSendStockModal();
        });
    ...
    function sendStockDetails(orderId) {
    var stockDetails = [];
    var canSubmit = true; 

    $('.stock_details tbody tr').each(function(index, row) {
        var quantity = parseInt($(row).find('.quantity-input').val());
        if (quantity > 0) {
            var stockItemId = $(row).find('.quantity-input').data('stock-item-id');
            var menuId = $(row).find('.quantity-input').data('menu-id');
            var stockDate = $(row).find('.quantity-input').data('stock-date');
            var expiryDate = $(row).find('.quantity-input').data('expiry-date');
            var orderItemId = $(row).find('.quantity-input').data('order-item-id');
            var resellerId = $(row).find('.quantity-input').data('reseller-id');
            
            var orderIdAttr = $(row).closest('.order-item').data('order-id');
            ...
            var totalQuantityInput = $(row).closest('.order-item').find('.total-quantity[data-menu-id="' + menuId + '"]');
            var totalQuantity = parseInt(totalQuantityInput.attr('data-total'));
            var maxQuantity = parseInt(totalQuantityInput.text().split('/')[1]); 
    ...
    // Additional check to ensure no totalQuantity is zero
    $('.total-quantity').each(function() {
        var totalQuantity = parseInt($(this).attr('data-total'));
        if (totalQuantity === 0) {
            canSubmit = false;
            return false;
        }
    ...
    if (canSubmit) {
        $.ajax({
            url: '<?php echo e(route("send.stock.details")); ?>',
            type: 'POST',
            data: {
                _token: '<?php echo e(csrf_token()); ?>',
                orderId: orderId, 
            ...
            dataType: 'json',
            success: function(response) {
                alert('Stock details sent successfully!');
            },
            error: function(xhr, status, error) {
                console.error('Error sending data:', error);
                alert('Failed to send stock details. Please try again.');
            }
        });
    } else {
        alert('Cannot submit: Total quantity does not match the order quantity');
    }
}

$('.send-button').click(function() {
    var orderId = $(this).data('order-id'); 
    sendStockDetails(orderId); 
});
...
<?php /**PATH C:\xampp\htdocs\pizzamatteo\resources\views/owner_stock_request.blade.php ENDPATH**/ ?>