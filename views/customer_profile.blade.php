...
<body>
    ...
    <div class="content-wrapper">
        <div class="custom-red">
            <p>Hi, {{ $userDetails->customer_name }}!</p>
        ...
        <div class="tab-content mt-4">
            <div id="myInformation" class="container tab-pane active">
                <div class="user-info">
                    <div class="column">
                        ...
                        <ul>
                            <li>Name: <span id="name">{{ $userDetails->customer_name }}</span></li>
                            <li>Birthday: <span id="birthday">{{ $userDetails->customer_birthday }}</span></li>
                            <li>Gender: <span id="gender">{{ $userDetails->customer_gender }}</span></li>
                        </ul>
    
                        <h2>Location Information</h2>
                        <ul>
                            <li>Province: <span id="province">{{ $userDetails->customer_province }}</span></li>
                            <li>Store: <span id="store">{{ $userDetails->store_name }}</span></li>
                            <li>City: <span id="city">{{ $userDetails->customer_city }}</span></li>
                            <li>Barangay: <span id="barangay">{{ $userDetails->customer_barangay }}</span></li>
                            <li>Street: <span id="street">{{ $userDetails->customer_street }}</span></li>
                        </ul>
                    </div>
                    <div class="column">
                        <h2>Contact Information</h2>
                        <ul>
                            <li>Phone Number: <span id="number">{{ $userDetails->customer_number }}</span></li>
                            <li>Email: <span id="email">{{ $userDetails->user_email }}</span></li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="container">
                    <div class="row">
                        @foreach ($customerOrders as $order)
                            @if ($order->order_status == 'Pending' || $order->order_status == 'Preparing' || $order->order_status == 'Ready for pickup')
                                    ...
                                    <div class="card custom-card">
                                        <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-center mb-3">
                                                <button class="btn btn-warning" onclick="openModal('orderDetailsPending{{ $order->order_id }}')">View Order</button>
                                            </div>
                                            <div class="text-center mb-3">
                                            ...
                                            <div class="d-flex justify-content-end">
                                                ...
                                                @elseif ($order->order_status == 'Ready for pickup')
                                                    <button class="btn btn-danger btn-sm" data-order-id="{{ $order->order_id }}" onclick="markOrderReceived('{{ $order->order_id }}')">Order Received</button>
                                                    <button class="btn btn-secondary btn-sm ml-2" onclick="openModal('refundModal{{ $order->order_id }}')">Request Refund</button>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="modal" id="orderDetailsPending{{ $order->order_id }}" style="display: none;">
                                    <div class="modal-content">
                                        <span class="close-btn" onclick="closeModal('orderDetailsPending{{ $order->order_id }}')">&times;</span>
                                        <h2 id="orderID">Order ID #{{ $order->order_id }}</h2>
                                        ...
                                        @php
                                            $totalAmount = 0;
                                        @endphp
                                        @foreach ($orderInformation->where('order_id', $order->order_id) as $info)
                                            <div class="flex-container">
                                                <div class="left-side">
                                                    <p>{{ $info->quantity }} - {{ $info->menu_name }} - [toast]</p>
                                                </div>
                                        ...
                                        @endforeach            
                                        <p style="text-align: right;">Order Total: ₱{{ $order->order_total_amount }}</p>
                                        <hr class="divider">
                                        <div class="modal-body">
                                            <p class="section-heading">Other Information</p>
                                            <p>Name: {{ $order->customer_name }}</p>
                                            <p>Store: {{ $order->store_name }}</p>
                                            <p>Purchase Type: {{ $order->purchase_type }}</p>
                                            <p>Payment Method: {{ $order->payment_type }}</p>
                                            <p>Amount Paid: {{ $order->amount_paid }}</p>
                                            <p>Gcash Name: {{ $order->gcash_name }}</p>
                                            <p>Gcash Number: {{ $order->gcash_number }}</p>
                                        </div>
                                ...
                                <!-- Refund Modal -->
                 <!-- Refund Modal -->
                 <div class="modal" id="refundModal{{ $order->order_id }}" style="display: none;">
                    <div class="modal-content">
                        <span class="close-btn" onclick="closeModal('refundModal{{ $order->order_id }}')">&times;</span>
                        <h2>Request Refund for Order ID #{{ $order->order_id }}</h2>
                
                        <form action="{{ route('refund.submit') }}" method="POST" enctype="multipart/form-data" class="refund-form">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                @foreach ($orderInformation->where('order_id', $order->order_id) as $info)
                                    <div class="form-check">
                                        <input class="form-check-input refund-checkbox" type="checkbox" name="refund_items[]" value="{{ $info->menu_id }}">
                                        <label class="form-check-label">
                                            {{ $info->menu_name }} - Quantity: {{ $info->quantity }} - Amount: ₱{{ $info->amount }}
                                        </label>
                                        <!-- Hidden inputs to capture quantity and amount -->
                                        <input type="hidden" name="refund_menu_ids[]" value="{{ $info->menu_id }}">
                                        <input type="hidden" name="refund_amounts[]" value="{{ $info->amount }}">
                                        <input type="number" class="form-control refund-quantity" name="refund_quantities[]" data-max="{{ $info->quantity }}" placeholder="Quantity" min="1" max="{{ $info->quantity }}">
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="row">
                        @foreach ($customerOrders as $order)
                            @if ($order->order_status == 'Completed')
                                <div class="col-md-4 col-sm-12 mb-3">
                                    <div class="card custom-card">
                                        <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-center mb-3">
                                                <button class="btn btn-warning" onclick="openModal('orderDetailsCompleted{{ $order->order_id }}')">View Order</button>
                                            </div>
                                            <div class="text-center mb-3">
                                                <p class="card-text">Status:{{ $order->order_status }}</p>
                                            </div>
                                ...
                                <div class="modal" id="orderDetailsCompleted{{ $order->order_id }}" style="display: none;">
                                    <div class="modal-content">
                                        <span class="close-btn" onclick="closeModal('orderDetailsCompleted{{ $order->order_id }}')">&times;</span>
                                        <h2 id="orderID">Order ID #{{ $order->order_id }}</h2>
                                        <div class="summary-section">
                                            <p>Order Summary</p>
                                        </div>
                    ...
                    <div class="row">
                        @foreach ($customerOrders as $order)
                            @if ($order->order_status == 'Cancelled' || $order->order_status == 'Refunded')
                                <div class="col-md-4 col-sm-12 mb-3">
                                    <div class="card custom-card">
                                        <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-center mb-3">
                                                <button class="btn btn-warning" onclick="openModal('orderDetailsCancelled{{ $order->order_id }}')">View Order</button>
                                            </div>
                                            <div class="text-center mb-3">
                                                <p class="card-text">Status: {{ $order->order_status }}</p>
                                            </div>
                                ...
                                <div class="modal" id="orderDetailsCancelled{{ $order->order_id }}" style="display: none;">
                                    <div class="modal-content">
                                        <span class="close-btn" onclick="closeModal('orderDetailsCancelled{{ $order->order_id }}')">&times;</span>
                                        <h2 id="orderID">Order ID #{{ $order->order_id }}</h2>
                                        <div class="summary-section">
                                            <p>Order Summary</p>
                                        </div>
            ...
            <div class="modal" id="refundModal{{ $order->order_id }}" style="display: none;">
                <div class="modal-content">
                    <span class="close-btn" onclick="closeModal('refundModal{{ $order->order_id }}')">&times;</span>
                    <h2 id="refundID">Refund Request for Order ID #{{ $order->order_id }}</h2>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="editName" name="customer_name" value="{{ $userDetails->customer_name }}">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="editGender">Gender</label>
                        <input type="text" class="form-control" id="editGender" name="customer_gender" value="{{ $userDetails->customer_gender }}">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="editBirthday">Birthday</label>
                        <input type="date" class="form-control" id="editBirthday" name="customer_birthday" value="{{ $userDetails->customer_birthday }}">
                    </div>
                </div>
            </div>          
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="editProvince" name="customer_province" onchange="updateStores()" required>
                            ...
                            @foreach ($province as $storeProvince)
                                <option value="{{ $storeProvince }}" @if($storeProvince == $userDetails->customer_province) selected @endif>
                                    {{ $storeProvince }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="editStore" name="customer_store" required>
                            ...
                            @foreach ($store as $s)
                                @if ($s->store_province == $userDetails->customer_province)
                                    <option value="{{ $s->reseller_id }}" @if($s->reseller_id == $userDetails->customer_store) selected @endif>
                                        {{ $s->store_name }}
                                    </option>
                                @endif
                            @endforeach
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="editCity" name="customer_city" value="{{ $userDetails->customer_city }}">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="editBarangay" name="customer_barangay" value="{{ $userDetails->customer_barangay }}">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="editStreet">Street</label>
                        <input type="text" class="form-control" id="editStreet" name="customer_street" value="{{ $userDetails->customer_street }}">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="editNumber">Phone Number</label>
                        <input type="text" class="form-control" id="editNumber" name="customer_number" value="{{ $userDetails->customer_number }}">
                    </div>
                </div>
            </div>
...
<div class="modal" id="changePassword" style="display: {{ $errors->any() ? 'block' : 'none' }};">
    <div class="modal-content">
...
$(document).ready(function() {
            ...
            function updateOrderStatus(orderId, status) {
                if (confirm("Are you sure you want to update the order status?")) {
                    $.ajax({
                        url: "{{ route('update.customer.order.status') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            orderId: orderId,
...
function togglePasswordVisibility(fieldId) {
        ...
        @foreach ($store as $s)
            if ("{{ $s->store_province }}" === province) {
                addOption(storeSelect, "{{ $s->store_name }}", "{{ $s->reseller_id }}");
            }
        @endforeach

        // Optionally, reselect the current store if it matches
        var currentStore = "{{ $userDetails->customer_store }}";
