            ...
            <div id="content-1" class="clickers">
                <table>
                    ...
                    <tbody id="table-1-body">
                        ...
                        <button class="btn-accept" data-order-id="{{ $order->order_id }}">Accept</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                    ...
                    <tbody id="table-3-body">
                        @foreach ($completedOrders as $order)
                            <tr>
                                ...
                                <td>{{ \Carbon\Carbon::parse($order->order_timestamp)->format('F j, Y, g:i A') }}</td>
                                <td>
                                    <button class="btn-summary" onclick="showCompleteSummaryModal('{{ $order->order_id }}')">See Order Summary</button>
                                </td>
                                <td>
                                    @if ($order->payment_proof)
                                        <button class="btn-proof" onclick="showPaymentProofModal('{{ asset('storage/' . $order->payment_proof) }}')">View Proof</button>
...
function showCompleteSummaryModal(orderId) {
    $.ajax({
        url: '{{ route("get.complete.summary") }}',
        type: 'GET',
...
function showDataInModal(orderId, targetContentId) {
    $.ajax({
        url: '{{ route("get.order.summary.and.inventory") }}',
        type: 'GET',
        data: { orderId: orderId },
        dataType: 'json',
        success: function(response) {
