                        ...
                        @if($stocksForMenu->isNotEmpty())
                            @foreach($stocksForMenu as $index => $stock)
                                ...
                                <tr>
                                    ...
                                    <td data-label="Stock Level" style="color: white; font-weight: bold; background-color: {{ $bgColor }};">
                                        {{ $stockStatus }}
                                    </td>
                                    <td>
            ...
            <tbody id="table-2-body">
                @foreach ($stockLog as $stock)
                    <tr data-stock-id="#{{ $stock->stock_item_id }}">
                        <td data-label="Stock ID">{{ $stock->stock_item_id }}</td>
                        <td data-label="Menu Name">{{ $stock->menu_name }}</td>
                        <td data-label="Price">₱{{ $stock->menu_base_price }}</td>
                        <td data-label="Quantity">{{ $stock->total_stock }}</td>
                        <td data-label="Total Price">₱{{ $stock->menu_base_price * $stock->total_stock }}</td>
                        <td data-label="Stock Date">{{ \Carbon\Carbon::parse($stock->stock_date)->format('F d, Y') }}</td>
                        <td data-label="Expiry Date">{{ \Carbon\Carbon::parse($stock->expiry_date)->format('F d, Y') }}</td>
                        <td data-label="Action">
                            <button type="button" class="btn-edit" data-toggle="modal" data-target="#editStockModal"
                            data-id="{{ $stock->stock_item_id }}" 
                            data-name="{{ $stock->menu_name }}"
                            data-quantity="{{ $stock->total_stock }}" 
                            data-expiry="{{ \Carbon\Carbon::parse($stock->expiry_date)->format('Y-m-d') }}">
                        <i class="fas fa-edit"></i> <!-- Font Awesome edit icon -->
                    </button>
                    <button type="button" class="btn-delete" data-id="{{ $stock->stock_item_id }}">
...
<div id="disposeModal" class="modal">
    <div class="modal-content">
        ...
        <h2 class="text-center">Dispose Stock</h2>
        <form id="disposeStockForm">
            ...
            <!-- Menu Name and Stock in the same row without space -->
            <div class="row">
                <div class="col-6">
                    <label for="disposeMenuName" style="margin-right: 0;">Menu Name:</label>
                    <p id="disposeMenuName" class="span-text" style="display: inline;"></p>
                </div>
                <div class="col-6">
                    <label for="disposeInventoryStock" style="margin-right: 0;">Stock:</label>
                    <p id="disposeInventoryStock" class="span-text" style="display: inline;"></p>
                </div>
            </div>
...
<div id="disposeModal" class="modal" style="display: none;">
    <div class="modal-content">
        ...
        <h2>Dispose Stock</h2>
        <form id="disposeStockForm" method="POST" enctype="multipart/form-data" action="{{ route('dispose.reseller.stock') }}">
            @csrf
