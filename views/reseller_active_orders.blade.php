                ...
                @foreach($customerOrders as $order)
                    @if($order->order_status === 'pending')
                        <tr>
                            ...
                            <td>
                                <button class="btn-proof" onclick="showPaymentProofModal('{{ asset('storage/' . $order->payment_proof) }}', '{{ $order->gcash_name }}', '{{ $order->gcash_number }}', '{{ $order->amount_paid }}')">View Proof</button>

                            <td>{{ $order->order_timestamp }}</td>
                            <td>{{ $order->order_expected_completion }}</td>
                            <td>
    ...
    <table>
        ...
        <tbody id="table-1-body">
            ...
            <tr>
                ...
                <td>{{ $order->customer_name }}</td>
                <td>{{ $order->purchase_type }}</td>
                <td><button class="btn-summary" onclick="showSummaryModal('{{ $order->order_id }}')">See Order</button></td>
                <td><button class="btn-proof" onclick="showPaymentProofModal('{{ asset('storage/' . $order->payment_proof) }}', '{{ $order->gcash_name }}', '{{ $order->gcash_number }}', '{{ $order->amount_paid }}')">View Proof</button></td>
                <td>{{ $order->order_timestamp }}</td>
                <td>{{ $order->order_expected_completion }}</td>
                <td>
                    <button class="btn-pickup" data-order-id="{{ $order->order_id }}">Pickup</button>
                </td>
            </tr>
            @endif
            @endforeach
                ...
                <tbody id="table-2-body">
                    ...
                    @if($order->order_status === 'Ready for pickup')
                        <tr>
                            ...
                            <td>{{ $order->order_expected_completion }}</td>
                            <td>
                                <button class="btn-cancel" data-order-id="{{ $order->order_id }}">Refund Request</button>
                            </td>
                        </tr>
                    @endif
                @endforeach
...
function showSummaryModal(orderId) {
    $.ajax({
        url: '{{ route("customer.order.summary") }}',
        type: 'GET',
